...
@Component({
  selector: 'app-orders-to-me',
  templateUrl: './orders-to-me.component.html',
  styleUrls: ['./orders-to-me.component.scss']
})
export class OrdersToMeComponent implements OnInit {
  ...
  getMyOrders(count){
    ...
    this.showLoading()
    this.auth.getUser().subscribe((response:any)=>{
      this.http.get(this.domain.url+'api/users/'+response.pk+'/orders-to-me/?limit=25&offset='+count*25,{headers:new HttpHeaders({'Authorization':'Token '+localStorage.getItem('user'),'Accept-Language':'az-Latn'})}).subscribe((res:any)=>{
